...
-- 3. Nhập vào 1 mã sinh viên và 1 mã môn học, nếu điểm trung bình môn học đó của sinh viên lớn hơn hoặc bằng 5 thì in ra "Đậu", 
-- nhỏ hơn 5 thì in ra "Không đậu", còn nếu SV chưa học môn đó thì in ra thông báo 
CREATE PROC kiemtra_dau_rot(@MSSV CHAR(8), @MSMH CHAR(5))
AS
BEGIN
		IF((SELECT DTB FROM SV_MH WHERE MSSV = @MSSV AND MSMH = @MSMH) >= 5)
			PRINT (N'ĐẬU') 
		ELSE IF NOT EXISTS (SELECT * FROM SV_MH WHERE MSSV = @MSSV AND MSMH = @MSMH)
			PRINT (N'SINH VIÊN CHƯA HỌC MÔN NÀY')
		ELSE
			PRINT (N'KHÔNG ĐẬU')
END
...
EXEC kiemtra_dau_rot @MSSV = '21520001', @MSMH = 'IT004' -- CHƯA HỌC 
...
-- DEMO SINH VIEN BỊ TU CHOI QUYEN TREN BANG SV_MH
USE QLSV
GO
--demo
USE QLSV
...
UPDATE SV_MH
SET DTB = 9.2
WHERE MSSV = '21520005' AND MSMH = 'IT005'
...
SET DATEFORMAT DMY
INSERT INTO SV_MH VALUES ('21520005','IE101','05/09/2022','31/12/2023',9.2);
...
SET DATEFORMAT DMY
INSERT INTO SINHVIEN VALUES ('21520006',N'Nguyễn Thanh Tú','0937446553',N'Quận 9','KH06');
